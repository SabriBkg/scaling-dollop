...
## Deferred from: code review of story-3-1-v1-dpa-acceptance-gate (2026-04-29)

- **Version-aware re-acceptance / carry-forward fingerprint loss** — `require_dpa_accepted()` checks `dpa_accepted` only, not `dpa_version == CURRENT_DPA_VERSION`; spec defines this behavior intentionally. But `accept_dpa` also drops `prior_dpa_version` on a reset path (no operator-reset audit trail). Revisit in a hardening pass. _Reason: behavior matches spec; legal-evidence enhancement._
- **Migration `0015` unbatched `UPDATE`** — `Account.objects.filter(dpa_accepted_at__isnull=False).update(dpa_version="v0-legacy")` is a single row-level write lock against `core_account`. Academic at zero-customer scale; will become a deploy hazard at growth. _Reason: scaling concern; rewrite as paginated cursor when account count exceeds ~100k._
- **Empty-string vs null `dpa_version` rolling-deploy race** — Backend translates `""` → `None` via `account.dpa_version or None`, but a row written between `AddField` and `RunPython` could surface as `dpa_version=""` then map to `null` for frontend, while frontend `??` fallback to `"v0-legacy"` only catches null/undefined. Narrow window. _Reason: consider `null=True` in a future migration to remove the dual-state ambiguity._
- **`useAccount` cross-session cache leak** — `staleTime: 5 min` + default `gcTime` plus user-agnostic queryKey `["account", "me"]` means logout/login as a different user can briefly surface the previous user's `dpa_accepted` value. Pre-existing. _Reason: add `queryClient.clear()` to logout flow._
- **`accept_dpa` missing `Cache-Control: no-store, private`** — Pattern established by Story 4.5 password-reset hardening; not applied to other auth-sensitive endpoints. _Reason: apply broadly across mutation endpoints._
- **`accept_dpa` double-fetch of `request.user.account`** — Outer fetch (line 152) is shadowed by `select_for_update` re-fetch (line 159); the outer is wasted. Pre-existing. _Reason: minor cleanup._
- **`NotificationPreview` opt-out token leakage to unsigned-DPA users** — Settings page now renders the preview for any non-Free account regardless of DPA status, generating a real signed opt-out URL keyed to `account.id`. Broader audience than before. _Reason: gate preview behind `dpa_accepted` in a follow-up, or generate a non-functional preview token._
- **Pre-existing error envelopes missing `field` key** — `TIER_NOT_ELIGIBLE`, `set_engine_mode`, `set_notification_tone` 403s return `{"error": {"code", "message"}}` without `"field": None`, violating the architecture envelope contract. Story 3.1 v1 explicitly out-of-scope. _Reason: broader Sprint Change Proposal §3.2 quarantine pass._
- **`dpa_version` format unpinned by tests** — Format `"vN.M-YYYY-MM-DD"` and sentinel `"v0-legacy"` are public contract via `_build_account_response`, but no regex/format test guards against drift. _Reason: hardening pass._

## Deferred from: code review of 3-2-v1-current-month-failed-payments-dashboard (2026-04-30)

- **UTC month boundary (no per-account timezone)** — `views/dashboard.py:237-238`. `month_start = now.replace(day=1, ...)` is computed in UTC; non-UTC users see ~1-day off-by-one at month edges. _Reason: explicit v2 spec deferral per Dev Notes §Timezone._
- **No pagination on `failed_payments_list`** — `views/dashboard.py:259-280`. Heavy accounts (>500 monthly failures) ship full list every 5 min via `refetchInterval`. _Reason: explicit v2 spec deferral per Dev Notes §v1 Scope Boundaries._
- **Hardcoded EUR currency** — `FailedPaymentsList.tsx:204`. `formatCurrency(amount_cents, "EUR")` is wrong for USD/GBP Stripe accounts. _Reason: explicit v2 spec deferral per Dev Notes §Currency._
- **`request.user.__class__.account.RelatedObjectDoesNotExist` pattern is fragile** — `views/dashboard.py:90`. Resolves through `__class__` descriptor; if `account` ever stops being a OneToOne, raises `AttributeError` at lookup. _Reason: pre-existing pattern from `dashboard_summary`, mandated by Task 1.2._
- **StatusBadge crashes on unknown subscriber_status** — `components/subscriber/StatusBadge.tsx`. `badgeConfig[status]` is undefined for unknown values, next access throws. _Reason: pre-existing component; defensive default belongs in a separate hardening pass._
- **Tooltip flicker for loading Free user** — `FailedPaymentsList.tsx:236-241`. Free user momentarily sees placeholder copy instead of tier copy before `useAccount` resolves. _Reason: sub-second flicker, low impact._
- **React Query cache key omits month boundary** — `useFailedPayments.ts:17-26`. User keeping page open across midnight UTC sees stale data up to 5 min after rollover. _Reason: narrow edge case; refetch covers within 5 min._
- **Native `title` attribute for tooltips instead of `@base-ui/react`** — `FailedPaymentsList.tsx:922`. Disabled buttons in some browsers don't show native tooltips reliably. _Reason: documented scope deviation in completion notes._
- **AC2 back/forward preservation not directly asserted in tests** — `__tests__/FailedPaymentsList.test.tsx:648-674`. Test asserts `router.replace` calls but not URL-state read-back into table on first render. _Reason: implicit coverage via `router.replace` tests._
- **`formatDate` uses `en-US` locale unconditionally** — `lib/formatters.ts:8-10`. EU users see `4/15/2026` instead of `15/04/2026` (especially confusing alongside EUR currency). _Reason: broader i18n concern, pre-existing utility._
- **`formatRelativeTime` returns negative values on server clock skew** — `lib/formatters.ts:12-21`. Clock skew >60s returns "-1d ago". _Reason: pre-existing utility, edge case._
- **A11y: `data-fraud="true"` has no consumer; no audible fraud announcement** — `FailedPaymentsList.tsx:187-188`. Color-only signal (amber border) for fraud rows. _Reason: a11y enhancement for follow-up._
- **Free-tier upgrade banner renders above empty state** — `FailedPaymentsList.tsx:250-267`. Empty Free account sees both the upgrade banner and "Your subscribers are paying" empty state simultaneously, awkward hierarchy. _Reason: UX polish._
- **`failed_payments_list` not cached server-side** — `views/dashboard.py:213-283`. Every dashboard mount hits DB even though FE has 5-min staleTime. _Reason: v2 perf optimization._
- **URL may accumulate `?upgrade=success` if sort click races dashboard cleanup effect** — `FailedPaymentsList.tsx:67-76`. Narrow timing window. _Reason: edge case._
- **Frontend test fixture provides partial Account (missing `dpa_version`, `engine_active`)** — `__tests__/FailedPaymentsList.test.tsx`. Type bypass via `as Account` cast hides drift from real type. _Reason: testing hygiene improvement._
- **Fraud row visual treatment may not paint with `border-collapse`** — `FailedPaymentsList.tsx:188`. `border-amber-500 border-2` on `<tr>` is commonly skipped by browsers when table uses `border-collapse`; shadcn `Table` typically uses `border-separate` so likely paints, but Task 9 manual smoke was deferred. _Reason: UX polishing will be done later on._
- **`useSortFromUrl.setSort` reads stale `useSearchParams` snapshot on rapid clicks** — `FailedPaymentsList.tsx:67-76`. `params` is captured at render; on rapid double-click within a single render frame the second click reads stale `sort`/`dir` and computes the wrong toggle. Initial fix using `window.location.search` regressed the existing sort tests because the test suite mocks `useSearchParams` only and not `window.location`. _Reason: bug is theoretical; revisit alongside test infra changes (mock `window.location`, or use a `useRef` re-sync pattern that stays compatible with the existing mock)._

## Deferred from: code review of 3-3-v1-per-row-send-and-manual-resolve (2026-04-30)

- **Duplicate-send race in `send_dunning_email`** — task calls Resend before writing `NotificationLog`; two concurrent tasks for the same `(failure, email_type)` both physically send before either log row is created. Pattern shared with all three pre-existing notification tasks. `backend/core/tasks/notifications.py:424-449`. _Reason: pre-existing across all four tasks; out-of-scope for 3.3._
- **`_throttled_message` picks first scoped throttle class arbitrarily** — `errors.py:13-21`. Works today (each view has one throttle), fragile if a future view stacks multiple throttle classes with different scopes. _Reason: hardening; not introduced by 3.3._
- **`request.user.account` missing returns 404 not 403** — `send_email.py:46,165`. Spec prescribed this exact code; matches existing convention in `actions.py`. _Reason: matches existing convention._
- **`test_password_reset.py` 429 message regression** — message changed from "Too many password reset requests…" to generic "Too many requests…". Out-of-scope side effect of the per-scope throttle refactor. _Reason: Story 4.5 owns the password-reset surface._
- **Migration 0017 (auditlog actor) unrelated to Story 3.3** — `0017_alter_auditlog_actor.py` completes Story 4.4's missing migration. Sabri-approved per Debug Log. _Reason: bisect hazard if 4.4 rolls back; flagged for awareness._
- **`useMarkResolved` error type is `Error`, not envelope-typed** — server returns INVALID_TRANSITION code with a specific message; frontend can't surface it. Toast falls back to generic "Failed to mark resolved." _Reason: minor UX; toast still informs._
- **`useSendEmail`/`useMarkResolved` don't invalidate `["actions","pending"]`** like `useExcludeSubscriber` does — minor stale-data risk on review-queue page. _Reason: covered by D3 decision-needed item; will be applied uniformly later._
- **Cross-action lockout in `ActionButtons`** — `FailedPaymentsList.tsx:183-184`. Any pending mutation disables all four buttons in the row. _Reason: mild UX friction; per-action pending state is a future improvement._
- **No test for throttle window reset (60s elapse → 11th passes)** — `test_send_email.py`. _Reason: 11th-fails path is covered; window-reset behavior would need freezegun or time-travel; nice-to-have._
- **`_throttled_message` `scope = None` initialization fragility** — `errors.py:15-21`. Works today; future-edit risk. _Reason: hardening._
- **`labelFor` falls back to raw type string** — minor i18n leak if backend adds new email types without frontend update. _Reason: cosmetic; future-proofing._
- **`retry-after` header uses bracket access on raw headers** — `useSendEmail.ts`. Works with Axios's lowercase normalization; brittle if interceptors swap headers. _Reason: works in practice._
- **Dropdown mock in tests bypasses Radix focus/keyboard semantics** — `FailedPaymentsList.test.tsx`. A11y regressions (Esc-to-close, arrow nav, focus trap) are completely untested. _Reason: test infra gap; not a product bug._
