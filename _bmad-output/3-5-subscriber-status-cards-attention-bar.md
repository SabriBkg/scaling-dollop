...
- [ ] Task 1: Create subscriber list API endpoint (AC: 1, 2, 3)
  - [ ] 1.1 Create `backend/core/serializers/subscribers.py` with `SubscriberCardSerializer` — fields: `id`, `stripe_customer_id`, `email`, `status`, `decline_code`, `decline_reason` (human label), `amount_cents`, `needs_attention` (boolean), `excluded_from_automation`
  - [ ] 1.2 Create `backend/core/views/subscribers.py` with `subscriber_list` GET endpoint
  - [ ] 1.3 Query: `Subscriber.objects.for_account(account_id)` with annotated latest failure info (decline_code, amount_cents) via subquery from `SubscriberFailure`
  - [ ] 1.4 Sorting: attention-state subscribers first (`fraud_flagged` status OR has pending action with `status=pending`), then by `updated_at` desc
  - [ ] 1.5 Register route: `path("v1/subscribers/", subscriber_list, name="subscriber_list")` in `core/urls.py`
  - [ ] 1.6 Tests in `backend/core/tests/test_api/test_subscribers.py`

- [ ] Task 2: Create attention summary API endpoint (AC: 4, 5, 6)
  - [ ] 2.1 Add `attention_items` list to the existing `dashboard_summary` response OR create a new lightweight endpoint `GET /api/v1/dashboard/attention/`
  - [ ] 2.2 Each attention item: `{ type: "fraud_flag" | "pending_action" | "retry_cap", subscriber_id: int, subscriber_name: string, label: string }`
  - [ ] 2.3 Sources: fraud-flagged subscribers (status=`fraud_flagged`), pending actions (Supervised mode, status=`pending`), retry cap approaching (failures where `retry_count >= retry_cap - 1` from rules)
  - [ ] 2.4 Update `DashboardSummarySerializer` to include `attention_items` field
  - [ ] 2.5 Tests for attention item aggregation

### Frontend

- [ ] Task 3: Create TypeScript types (AC: 1, 2)
  - [ ] 3.1 Create `frontend/src/types/subscriber.ts` with `SubscriberCard` interface: `id`, `stripe_customer_id`, `email`, `status` (union: `'active' | 'recovered' | 'passive_churn' | 'fraud_flagged'`), `decline_code`, `decline_reason`, `amount_cents`, `needs_attention`, `excluded_from_automation`
  - [ ] 3.2 Create `AttentionItem` interface in `frontend/src/types/dashboard.ts`: `type`, `subscriber_id`, `subscriber_name`, `label`
  - [ ] 3.3 Update `DashboardSummary` interface to include `attention_items: AttentionItem[]`

- [ ] Task 4: Create `useSubscribers` hook (AC: 1, 3)
  - [ ] 4.1 Create `frontend/src/hooks/useSubscribers.ts` — TanStack Query hook, `GET /api/v1/subscribers/`, queryKey `["subscribers"]`, `refetchInterval: 5 * 60 * 1000`

- [ ] Task 5: Build `StatusBadge` component (AC: 2)
  - [ ] 5.1 Create `frontend/src/components/subscriber/StatusBadge.tsx`
  - [ ] 5.2 4 variants with correct colors: Recovered (green `--accent-recovery`), Active (blue `--accent-active`), Fraud Flagged (red `--accent-fraud`), Passive Churn (grey `--accent-neutral`)
  - [ ] 5.3 Always render text label alongside color — never color-only
  - [ ] 5.4 Font: Inter 500 (Medium), 11-12px per UX spec
  - [ ] 5.5 Include `aria-label` with full status text

- [ ] Task 6: Build `SubscriberCard` component (AC: 1, 2, 3, 8)
  - [ ] 6.1 Create `frontend/src/components/dashboard/SubscriberCard.tsx`
  - [ ] 6.2 Layout: top row = name (bold) + amount (right-aligned); sub-label = email; bottom row = plain-language decline reason + StatusBadge
  - [ ] 6.3 States: Default, Hover (border darkens), Attention (amber border + "⚠" prefix on reason text), Recovered (green amount)
  - [ ] 6.4 Fraud-flagged cards: amber border + "⚠ Fraud flagged" label
  - [ ] 6.5 Clicking opens subscriber detail (future story — wire to navigation or noop for now)

- [ ] Task 7: Build `SubscriberGrid` component (AC: 1, 3)
  - [ ] 7.1 Create `frontend/src/components/dashboard/SubscriberGrid.tsx`
  - [ ] 7.2 Renders list of SubscriberCard components
  - [ ] 7.3 Client-side sorting guarantee: attention-state cards first (fraud_flagged status, needs_attention=true), then rest by status
  - [ ] 7.4 Loading skeleton state (`SubscriberGridSkeleton`)

- [ ] Task 8: Build `AttentionBar` component (AC: 4, 5, 6, 7)
  - [ ] 8.1 Create `frontend/src/components/dashboard/AttentionBar.tsx`
  - [ ] 8.2 Amber strip below topbar: warning icon (left), "N items need your attention" (bold, dark amber), "Review before next engine cycle in Xm" (lighter amber), action pills (right)
  - [ ] 8.3 Action pills: named clickable chips, one per attention item — clicking navigates to relevant subscriber or review queue
  - [ ] 8.4 Hidden when `attention_items` is empty
  - [ ] 8.5 `role="alert"` with `aria-live="polite"` for accessibility
  - [ ] 8.6 States: Visible (1+ items), Hidden (0 items), Urgent (fraud flag present — slightly deeper amber)

- [ ] Task 9: Integrate into Dashboard page (AC: all)
  - [ ] 9.1 Add `AttentionBar` to dashboard layout — renders between NavBar and main content (conditionally)
  - [ ] 9.2 Add `SubscriberGrid` below existing StoryArcPanel and DeclineBreakdown sections
  - [ ] 9.3 Wire `useSubscribers` hook data into SubscriberGrid
  - [ ] 9.4 Wire attention items from `useDashboardSummary` into AttentionBar
  - [ ] 9.5 Calculate "next engine cycle" countdown from `account.next_scan_at`

- [ ] Task 10: Frontend tests (AC: all)
  - [ ] 10.1 `frontend/src/__tests__/SubscriberCard.test.tsx` — renders all 4 status variants, attention styling, fraud amber border
  - [ ] 10.2 `frontend/src/__tests__/AttentionBar.test.tsx` — renders when items exist, hidden when empty, correct pill rendering, accessibility attributes
  - [ ] 10.3 `frontend/src/__tests__/StatusBadge.test.tsx` — all 4 variants with correct colors and text labels
